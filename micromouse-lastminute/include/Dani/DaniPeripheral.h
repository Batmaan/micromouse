...
uint32_t ADC1ConvertedValue[2] =
{ 0, 0 };
uint32_t ADC2ConvertedValue[2] =
{ 0, 0 };
...
void MotorInit(void)
{
    ...
    TIM_OCStruct.TIM_Pulse = 0; /* CH2 jobbmotor */
    TIM_OC2Init(TIM3, &TIM_OCStruct);
    TIM_OC2PreloadConfig(TIM3, TIM_OCPreload_Enable);

    TIM_OCStruct.TIM_Pulse = 0; /* CH4 balmotor */
...
void ADCInit(void)
{
	...
	//DMA CH2
	//
	DMA_InitStruct.DMA_PeripheralBaseAddr = (uint32_t) & ADC2->DR; //ADC13's data register ugye ahonnan
	DMA_InitStruct.DMA_MemoryBaseAddr = (uint32_t) &ADC2ConvertedValue; // globális változó pointer
	DMA_InitStruct.DMA_DIR = DMA_DIR_PeripheralSRC; //peripherial source,  nem destination
	DMA_InitStruct.DMA_BufferSize = 2; //2 változót ir ugye a tömbbe
	DMA_InitStruct.DMA_PeripheralInc = DMA_PeripheralInc_Disable; //ez nemtudmo mi
	DMA_InitStruct.DMA_MemoryInc = DMA_MemoryInc_Enable; //ez sem
	DMA_InitStruct.DMA_PeripheralDataSize = DMA_PeripheralDataSize_HalfWord; //Reads 16 bit values
	DMA_InitStruct.DMA_MemoryDataSize = DMA_MemoryDataSize_HalfWord; //Stores 16 bit values
	DMA_InitStruct.DMA_Mode = DMA_Mode_Circular; // mégjobb kérdés
	DMA_InitStruct.DMA_Priority = DMA_Priority_High; //ez viszont elég érthetõ
